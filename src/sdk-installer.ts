...
export async function installAndroidSdk(
  ...
  try {
    ...
    if (!fs.existsSync(cmdlineToolsPath)) {
      ...
      await io.mv(
        `${cmdlineToolsPath}/cmdline-tools`,
        `${cmdlineToolsPath}/latest`
      )
    }

    // add paths for commandline-tools and platform-tools
    core.addPath(
      `${cmdlineToolsPath}/latest:${cmdlineToolsPath}/latest/bin:${process.env.ANDROID_HOME}/platform-tools`
    )
    ...
    await exec.exec(
      `sh -c \\"sdkmanager --install emulator --channel=${channelId} > /dev/null"`
    )

    if (emulatorBuild) {
      console.log(`Installing emulator build ${emulatorBuild}.`)
      // TODO find out the correct download URLs for all build ids
      let downloadUrlSuffix: string
      ...
      await exec.exec(
        `curl -fo emulator.zip https://dl.google.com/android/repository/emulator-${isOnMac ? 'darwin' : 'linux'}${downloadUrlSuffix}.zip`
      )
    ...
    await exec.exec(
      `sh -c \\"sdkmanager --install 'system-images;android-${systemImageApiLevel};${target};${arch}' --channel=${channelId} > /dev/null"`
    )

    if (ndkVersion) {
      console.log(`Installing NDK ${ndkVersion}.`)
      await exec.exec(
        `sh -c \\"sdkmanager --install 'ndk;${ndkVersion}' --channel=${channelId} > /dev/null"`
      )
    }
    if (cmakeVersion) {
      console.log(`Installing CMake ${cmakeVersion}.`)
      await exec.exec(
        `sh -c \\"sdkmanager --install 'cmake;${cmakeVersion}' --channel=${channelId} > /dev/null"`
      )
    }
  } finally {
