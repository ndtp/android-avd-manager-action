...
export function checkArch(arch: string): void {
  if (!VALID_ARCHS.includes(arch)) {
    throw new Error(
      `Value for input.arch '${arch}' is unknown. Supported options: ${VALID_ARCHS}.`
    )
  }
}
...
export function checkChannel(channel: string): void {
  if (!VALID_CHANNELS.includes(channel)) {
    throw new Error(
      `Value for input.channel '${channel}' is unknown. Supported options: ${VALID_CHANNELS}.`
    )
  }
}
...
export function checkForceAvdCreation(forceAvdCreation: string): void {
  if (!isValidBoolean(forceAvdCreation)) {
    throw new Error(
      `Input for input.force-avd-creation should be either 'true' or 'false'.`
    )
  }
}

export function checkEnableHardwareKeyboard(
  enableHardwareKeyboard: string
): void {
  if (!isValidBoolean(enableHardwareKeyboard)) {
    throw new Error(
      `Input for input.enable-hw-keyboard should be either 'true' or 'false'.`
    )
  }
}
...
export function checkEmulatorBuild(emulatorBuild: string): void {
  if (
    isNaN(Number(emulatorBuild)) ||
    !Number.isInteger(Number(emulatorBuild))
  ) {
...
export function checkDiskSize(diskSize: string): void {
  ...
  if (diskSize) {
    ...
    if (isNaN(Number(diskSize)) || !Number.isInteger(Number(diskSize))) {
      ...
      if (
        diskSizeUpperCase.endsWith('K') ||
        diskSizeUpperCase.endsWith('M') ||
        diskSizeUpperCase.endsWith('G')
      ) {
        const diskSizeNoModifier: string = diskSize.slice(0, -1)
        if (
          0 == diskSizeNoModifier.length ||
          isNaN(Number(diskSizeNoModifier)) ||
